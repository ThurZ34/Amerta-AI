...
@section('content')
        ...
        <div class="max-w-4xl mx-auto space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 relative overflow-hidden group hover:border-emerald-200 dark:hover:border-emerald-900 transition-colors">
                    ...
                    <div class="absolute bottom-0 left-0 h-1 w-full bg-emerald-500 transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left"></div>
                </div>

                <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 relative overflow-hidden group hover:border-rose-200 dark:hover:border-rose-900 transition-colors">
                    <div class="flex items-center justify-between relative z-10">
                        <div>
                    ...
                    <div class="absolute bottom-0 left-0 h-1 w-full bg-rose-500 transform scale-x-0 group-hover:scale-x-100 transition-transform origin-left"></div>
                </div>
            </div>
                ...
                <div class="flex gap-2 w-full sm:w-auto">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" x-transition.origin.top.right class="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 z-50 overflow-hidden" style="display: none;">
                            ...
                            <button @click="openAddModal('pendapatan'); open = false" class="w-full text-left px-4 py-3 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center gap-2 border-t border-gray-100 dark:border-gray-700">
                ...
                <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50 flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    <div class="flex p-1 bg-gray-200 dark:bg-gray-700 rounded-lg">
                        <button @click="filterType = 'all'; currentPage = 1"
                            ...
                            :class="filterType === 'all' ? 'bg-white dark:bg-gray-600 text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'">
                            Semua
                        </button>
                        <button @click="filterType = 'pendapatan'; currentPage = 1"
                            class="px-4 py-1.5 text-xs font-semibold rounded-md transition-all"
                            :class="filterType === 'pendapatan' ? 'bg-white dark:bg-gray-600 text-emerald-600 dark:text-emerald-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'">
                            Masuk
                        </button>
                        <button @click="filterType = 'pengeluaran'; currentPage = 1"
                            class="px-4 py-1.5 text-xs font-semibold rounded-md transition-all"
                            :class="filterType === 'pengeluaran' ? 'bg-white dark:bg-gray-600 text-rose-600 dark:text-rose-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'">
                            Keluar
                        </button>
                    </div>
                </div>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-4">
                                    <div class="w-10 h-10 rounded-full flex items-center justify-center shrink-0"
                                        ...
                                        <svg x-show="riwayat.jenis === 'pendapatan'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                    </div>

                                    <div>
                                        <h4 class="text-sm font-bold text-gray-900 dark:text-white truncate max-w-[150px] sm:max-w-xs" x-text="riwayat.nama_barang"></h4>
                                        <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                            ...
                                            <template x-if="riwayat.kategori">
                                                <span class="px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-[10px]" x-text="riwayat.kategori"></span>
                                            </template>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="text-right">
                                    ...
                                    <div x-show="riwayat.is_manual === false" class="mt-1">
                                        <span class="text-[10px] bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded border border-indigo-100">Otomatis</span>
                                    </div>
                                </div>
                            </div>
                        </li>
                    ...
                    <form :action="isEditing ? `{{ route('operasional.riwayat-keuangan.index') }}/${editId}` : '{{ route('operasional.riwayat-keuangan.store') }}'" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            ...
                            <div x-show="formData.jenis === 'pengeluaran'">
                                ...
                                <input type="text" name="kategori" x-model="formData.kategori" list="categoryList" class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500">
                                <datalist id="categoryList"><template x-for="cat in categories" :key="cat"><option :value="cat"></option></template><option value="Bahan Baku"></option><option value="Operasional"></option><option value="Gaji"></option></datalist>
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tanggal</label><input type="date" name="tanggal_pembelian" x-model="formData.tanggal_pembelian" required class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500"></div>
                                <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nominal (Rp)</label><input type="hidden" name="total_harga" x-model="formData.total_harga"><input type="text" @input="updateTotalHarga($event)" :value="displayTotalHarga" required class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 font-mono"></div>
                            </div>
                            <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Bukti (Opsional)</label><input type="file" name="bukti_pembayaran" accept="image/*" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100"></div>
                            <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Catatan</label><textarea name="keterangan" x-model="formData.keterangan" rows="2" class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 resize-none"></textarea></div>
                        </div>

                        <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 flex justify-end gap-3">
                            <button type="button" @click="showModal = false" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 dark:bg-gray-700 dark:text-white dark:border-gray-600 dark:hover:bg-gray-600">Batal</button>
                            <button type="submit" class="px-4 py-2 text-sm font-bold text-white rounded-lg hover:shadow-lg transition-all" :class="formData.jenis === 'pengeluaran' ? 'bg-rose-600 hover:bg-rose-700' : 'bg-emerald-600 hover:bg-emerald-700'">Simpan</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('financeApp', ({ riwayats, scanResult, categories }) => ({
                riwayats: riwayats,
                ...
                get totalPengeluaran() {
                    return this.riwayats.filter(r => r.jenis === 'pengeluaran').reduce((acc, curr) => acc + parseFloat(curr.total_harga), 0);
                },
                get totalPendapatan() {
                ...
                handleScanResult(scanData) {
                    this.formData = {
                        ...
                        tanggal_pembelian: scanData.transaction_date || new Date().toISOString().split('T')[0],
                ...
                openAddModal(jenis) {
                    this.formData = { nama_barang: '', kategori: '', tanggal_pembelian: new Date().toISOString().split('T')[0], total_harga: '', keterangan: '', bukti_pembayaran: '', jenis: jenis };
                    this.isEditing = false;
                    this.showModal = true;
                },

                openEditModal(riwayat) {
                    this.formData = { ...riwayat };
                    this.isEditing = true;
                ...
                get displayTotalHarga() { return this.formData.total_harga ? 'Rp ' + this.formatRupiah(this.formData.total_harga) : ''; },
                updateTotalHarga(event) {
                    const rawValue = event.target.value.replace(/[^0-9]/g, '');
                    this.formData.total_harga = rawValue;
                    event.target.value = 'Rp ' + this.formatRupiah(rawValue);
                },
                triggerScan() { document.getElementById('scanInput').click(); },
                submitScan() { document.getElementById('scanForm').submit(); }
            }));
        });
    </script>
@endsection
