...
@section('content')
    <div class="min-h-screen bg-gray-50/50 dark:bg-gray-900 py-8 transition-colors duration-300 font-sans"
         x-data="{
            ...
            openImageModal(url) { this.modalImageUrl = url; this.showImageModal = true; },
            updatePreview(event) {
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if (session('success'))
                <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
                    x-transition class="mb-6 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 px-4 py-3 rounded-xl flex items-center gap-3 shadow-sm" role="alert">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <form action="{{ route('profil_bisnis.update') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="relative z-10 flex flex-col sm:flex-row items-start gap-6">
                                ...
                                <div class="group relative shrink-0">
                                    ...
                                    <div class="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl overflow-hidden border-4 border-white dark:border-gray-700 shadow-md bg-gray-100 dark:bg-gray-800 relative"
                                        ...
                                        <div x-show="isEditing" @click="document.getElementById('gambar-input').click()" class="absolute inset-0 bg-black/50 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer backdrop-blur-[1px]">
                                    ...
                                    <button type="button" x-show="isEditing && imagePreview" @click="removeImage()" class="absolute -top-2 -right-2 bg-white dark:bg-gray-700 text-red-500 rounded-full p-1.5 shadow-md border border-gray-100 dark:border-gray-600 hover:bg-red-50 transition-colors z-20"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
                                </div>
                                ...
                                <div class="flex-1 w-full space-y-4">
                                    <div>
                                        ...
                                        <template x-if="!isEditing">
                                            <h2 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white leading-tight">{{ optional($business)->nama_bisnis ?? 'Belum ada nama' }}</h2>
                                        </template>
                                        <template x-if="isEditing">
                                            <input type="text" name="nama_bisnis" value="{{ old('nama_bisnis', optional($business)->nama_bisnis ?? '') }}" class="w-full text-xl font-bold text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-lg focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 px-3 py-2 transition-all">
                                        </template>
                                    ...
                                    <div class="relative max-w-sm">
                                        ...
                                        <template x-if="!isEditing">
                                            <span class="inline-flex items-center px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-sm font-medium border border-indigo-100 dark:border-indigo-800">{{ optional(optional($business)->category)->name ?? 'Belum ada kategori' }}</span>
                                        </template>
                                        <template x-if="isEditing">
                                            <div class="relative z-50">
                                                <input type="hidden" name="kategori" :value="selectedCategory">
                                                <input type="text" x-model="categorySearch" @focus="showCategoryDropdown = true" @click.away="showCategoryDropdown = false" :placeholder="selectedCategory || 'Cari Kategori...'" class="w-full pl-10 pr-4 py-2.5 rounded-lg bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm text-gray-900 dark:text-white transition-all">
                                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><svg class="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg></div>

                                                <div x-show="showCategoryDropdown && (filteredCategories.length > 0 || showAddButton)" x-transition class="absolute z-[100] w-full mt-2 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl shadow-xl max-h-56 overflow-y-auto custom-scrollbar">
                                                    <template x-for="category in filteredCategories" :key="category">
                                                        <button type="button" @click="selectCategory(category)" class="w-full text-left px-4 py-2.5 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 text-sm text-gray-700 dark:text-gray-200 transition-colors" x-text="category"></button>
                                                    </template>
                                                    <template x-if="showAddButton">
                                                        <button type="button" @click="addNewCategory()" class="w-full text-left px-4 py-2.5 bg-indigo-50 dark:bg-indigo-900/10 hover:bg-indigo-100 dark:hover:bg-indigo-900/30 text-sm text-indigo-600 dark:text-indigo-400 font-medium border-t border-gray-100 dark:border-gray-700 flex items-center gap-2">
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 h-full">
                            ...
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <template x-if="!isEditing">
                                        <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed bg-gray-50 dark:bg-gray-700/30 p-4 rounded-xl border border-gray-100 dark:border-gray-700">{{ $business?->tujuan_utama ?: 'Belum diisi' }}</p>
                                    </template>
                                    <template x-if="isEditing">
                                        <textarea name="tujuan_utama" rows="3" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-gray-900 dark:text-white text-sm px-3 py-2.5 placeholder-gray-400">{{ old('tujuan_utama', $business?->tujuan_utama) }}</textarea>
                                    </template>
                                </div>
                                <div>
                                    ...
                                    <template x-if="isEditing">
                                        <textarea name="alamat" rows="3" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-gray-900 dark:text-white text-sm px-3 py-2.5 placeholder-gray-400">{{ old('alamat', $business?->alamat) }}</textarea>
                                    </template>
                                </div>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label class="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold tracking-wider">Target Pasar</label>
                                        <template x-if="!isEditing">
                                            <p class="font-medium text-gray-800 dark:text-gray-200 mt-1">{{ optional($business)->target_pasar ?? '-' }}</p>
                                        </template>
                                        <template x-if="isEditing">
                                            <input type="text" name="target_pasar" value="{{ old('target_pasar', $business?->target_pasar) }}" class="mt-1 w-full rounded-lg border border-gray-300 dark:border-gray-600 text-sm bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 px-3 py-2.5">
                                        </template>
                                    </div>
                                    <div>
                                        <label class="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold tracking-wider">Telepon / WA</label>
                                        <template x-if="!isEditing">
                                            <p class="font-medium text-gray-800 dark:text-gray-200 font-mono mt-1">{{ optional($business)->telepon ?? '-' }}</p>
                                        </template>
                                        <template x-if="isEditing">
                                            <input type="text" name="telepon" value="{{ old('telepon', $business?->telepon) }}" class="mt-1 w-full rounded-lg border border-gray-300 dark:border-gray-600 text-sm bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 px-3 py-2.5">
                                        </template>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <div class="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-2xl shadow-lg p-6 text-white relative overflow-hidden">
                        ...
                        <div class="flex items-center gap-2" x-data="{ copied: false }">
                            <code class="flex-1 bg-white/20 backdrop-blur-sm text-white border border-white/20 px-4 py-3 rounded-xl text-lg font-mono font-bold tracking-widest text-center shadow-inner">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 flex justify-between items-center">
                            ...
                            <span class="bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 text-xs font-bold px-2 py-1 rounded-md">{{ $business->users->count() }} Orang</span>
                        </div>
                        ...
                        <div class="divide-y divide-gray-100 dark:divide-gray-700">
                            ...
                            @if($staffs->count() > 0)
                                <div class="p-4">
                                    <p class="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2 ml-1">Staff ({{ $staffs->count() }})</p>
                                    <div class="space-y-3">
