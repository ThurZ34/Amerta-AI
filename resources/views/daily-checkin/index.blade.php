...
@section('content')
    ...
    <div class="p-4 sm:p-6 lg:p-8 h-full"
         x-data="{
            ...
            // Fungsi Buka Modal
            openInputModal(dateYmd, dateReadable) {
                this.targetDate = dateYmd;
                this.targetDateDisplay = dateReadable;
                this.createModalOpen = true;
            },

            // Fungsi Tutup Modal & Reset (Opsional jika mau reset form via JS, tapi reload page via server lebih aman buat data)
            closeModal() {
        ...
        @php
            // ... (Logic PHP Perhitungan Ringkasan TETAP SAMA seperti sebelumnya) ...
                    ...
                    <div class="grid grid-cols-7 bg-gray-200 dark:bg-gray-700 gap-px flex-1">
                        ...
                        @while ($currentDate <= $lastDate)
                            @php
                                ...
                                $dateReadable = $currentDate->translatedFormat('l, d F Y'); // Format terbaca untuk modal
                ...
                <div x-show="createModalOpen"
                    ...
                    <form action="{{ route('daily-checkin.store') }}" method="POST" @submit="isLoading = true">
                        ...
                        <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50 dark:bg-gray-800">
                            ...
                            <button type="button" @click="closeModal()" class="text-gray-400 hover:text-gray-500 transition-colors rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 p-1">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                    ...
                                    <div x-data="{ count: 0 }"
                                        ...
                                        <div class="flex-1 min-w-0 flex flex-col justify-between h-16">
                                            ...
                                            <div class="flex items-center justify-end">
                                                <div class="flex items-center bg-gray-50 dark:bg-gray-700/50 rounded-lg p-0.5 border border-gray-200 dark:border-gray-600">
                                                    <button type="button" @click="if(count > 0) count--" class="w-6 h-6 flex items-center justify-center rounded text-gray-500 hover:bg-white dark:hover:bg-gray-600 hover:text-indigo-600 transition-all disabled:opacity-50">
                                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M20 12H4" /></svg>
                                                    </button>

                                                    <input type="number" name="sales[{{ $produk->id }}]" x-model="count" min="0" class="w-8 text-center bg-transparent border-none p-0 text-sm font-bold text-gray-800 dark:text-white focus:ring-0 appearance-none [-moz-appearance:_textfield] [&::-webkit-inner-spin-button]:m-0 [&::-webkit-inner-spin-button]:appearance-none">

                                                    <button type="button" @click="count++" class="w-6 h-6 flex items-center justify-center rounded text-gray-500 hover:bg-white dark:hover:bg-gray-600 hover:text-indigo-600 transition-all">
                                ...
                                @empty
                                    <div class="col-span-full py-8 flex flex-col items-center justify-center text-center">
                                        <p class="text-gray-500 dark:text-gray-400 text-sm">Belum ada produk. Tambahkan di Katalog dulu.</p>
                                    </div>
                                @endforelse
                            </div>
                        </div>
                        ...
                        <div class="px-6 py-4 bg-gray-50 dark:bg-gray-800/50 flex justify-end gap-3 border-t border-gray-100 dark:border-gray-700">
                            ...
                            <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 shadow-sm shadow-indigo-500/30 transition flex items-center gap-2">
