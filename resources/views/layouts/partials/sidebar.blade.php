<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
    ...
    <div class="flex-1 overflow-y-auto py-6 px-4 space-y-1 custom-scrollbar bg-white dark:bg-gray-950">
        ...
        <a href="{{ route('daily-checkin.index') }}"
            class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200 group
            {{ request()->routeIs('daily-checkin.*')
                ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-600 dark:text-white'
                : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white' }}">
            <svg class="w-5 h-5 {{ request()->routeIs('daily-checkin.*') ? 'text-indigo-700 dark:text-white' : 'text-gray-400 dark:text-gray-500 group-hover:text-indigo-600 dark:group-hover:text-white' }} transition-colors"
                fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <a href="{{ route('riwayat.kasir') }}"
            class="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all duration-200 group
            {{ request()->routeIs('riwayat.kasir')
                ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-600 dark:text-white'
                : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white' }}">
            <svg class="w-5 h-5 {{ request()->routeIs('riwayat.kasir') ? 'text-indigo-700 dark:text-white' : 'text-gray-400 dark:text-gray-500 group-hover:text-indigo-600 dark:group-hover:text-white' }} transition-colors"
                fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @if(auth()->user()->role === 'owner' && auth()->user()->business && auth()->user()->id === auth()->user()->business->user_id)
            <p class="px-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Pengaturan</p>
            ...
            <a href="{{ route('profil_bisnis') }}"
                ...
                {{ request()->routeIs('profil_bisnis')
                    ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-600 dark:text-white'
                    : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white' }}">
                <svg class="w-5 h-5 {{ request()->routeIs('profil_bisnis') ? 'text-indigo-700 dark:text-white' : 'text-gray-400 dark:text-gray-500 group-hover:text-indigo-600 dark:group-hover:text-white' }} transition-colors"
                     fill="none" viewBox="0 0 24 24" stroke="currentColor">
